...
from artifact_memory.coordination_outbox_conformance import (
    CONFORMANCE_SCHEMA_ID,
    render_coordination_outbox_conformance_receipt,
    run,
)
...
FIXTURE = ROOT / "fixtures" / "coordination-outbox" / "v0"


class CoordinationOutboxConformanceTests(unittest.TestCase):
    def test_checked_receipt_is_registered_valid_and_rendered(self):
        ...
        self.assertEqual(
            render_coordination_outbox_conformance_receipt(receipt),
    ...
    def test_check_rejects_machine_receipt_drift(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            completed = subprocess.run(
                [
                    sys.executable,
                    "scripts/run_coordination_outbox_conformance.py",
                    "--fixture",
                    str(fixture),
                    "--check",
                ],
            ...
            self.assertIn(
                "coordination outbox conformance receipt does not match checked evidence",
                completed.stderr,
            )
