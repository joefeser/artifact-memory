...
def inspect_zip(path: Path, max_uncompressed_bytes: int = 16 * 1024 * 1024) -> dict[str, Any]:
    ...
    total = 0
    try:
        container_digest = sha256_path(path)
        ...
        return {"schema_id": "artifact-memory/archive-receipt/v1", "outcome": "failed", "container_digest": "sha-256:" + "0" * 64, "entries": [], "limitations": ["archive could not be inspected"], "diagnostics": [{"code": "corrupt", "message": "archive container is invalid"}]}
