...
an explicit `unstable` diagnostic and contributes no content digest. Manifest
admission validates schema, sorted unique normalized paths, parent-directory
coverage, file/directory field invariants, tree digest, and manifest identity.
...
rather than treated as comparable scopes. A moved candidate is a content/tree match only; it
does not prove semantic continuity, ownership, or source-language meaning.
