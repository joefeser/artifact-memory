...
- Artifact Memory release or commit expected by the operator: <AM_VERSION>
...
3. Run `artifact-memory version --json` and report whether it matches
   <AM_VERSION>.
4. Run `artifact-memory validate "<CONTEXT_PACK_PATH>" --json`. Stop on any
   unsupported schema, validation failure, identity mismatch, or byte-bound
   failure.
