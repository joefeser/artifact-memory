...
the owner-signed v0.1.3 release.
...
Implement one provider-free Artifact Memory v0.1.3 conformance proof governed
by <ISSUE_URL>.

Pinned inputs:
- Fetch and verify exact <ADOPTING_REPOSITORY_REF>; report its SHA before
  editing.
- Pin Artifact Memory to owner-signed tag v0.1.3, record the resolved commit,
...
The v0.1.3 exporter supports frozen context-pack v2/v3 and negotiated v4. V4 is
...
Consume one synthetic Artifact Memory v0.1.3 context pack in
...
<ADOPTING_REPOSITORY_SHA> against owner-signed Artifact Memory v0.1.3.
...
Release. The signed `v0.1.3` tag and its published assets remain immutable.
