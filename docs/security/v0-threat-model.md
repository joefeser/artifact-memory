...
| Generated SQLite projection is tampered in place (M1) | Read gate: exact-contract validation, `PRAGMA integrity_check`, runtime floor, and one read transaction per query (decision 0023) | Requires local write access to the projection file — the same trust boundary as any local cache; the gate converts silent forgery into a typed `projection-unavailable`, canonical records are unaffected, and v0 exposes no network surface to the projection; runtimes below SQLite 3.44 fail closed rather than serving unverifiable reads |
