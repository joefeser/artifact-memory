...
| Generated SQLite projection is tampered in place (M1) | Read gate: exact-contract validation, `PRAGMA integrity_check`, a behavioral FTS5 capability probe, and one read transaction per query (decision 0023) | Requires local write access to the projection file — the same trust boundary as any local cache; the gate converts silent forgery into a typed `projection-unavailable`, canonical records are unaffected, and v0 exposes no network surface to the projection; incapable loaded builds fail closed rather than serving unverifiable reads |
