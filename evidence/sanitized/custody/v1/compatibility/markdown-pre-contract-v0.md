...
- Restored verification: ten allowlisted source files matched the private
  manifest; four canonical artifact/version records validated; two content
  objects matched their digests; the restored Git bundle verified
- Storage boundary: ZFS-backed repository with a separately controlled weekly
  snapshot timer and bounded retention; a manual server-controlled post-write
  snapshot succeeded after the first remote backup
- Recovery cadence: monthly restic integrity verification and quarterly
